import { heading, link, unorderedList } from '../../../libs/markdown/index.js';
...
export function memberWithGroups(model, options) {
    ...
    if (model.typeHierarchy?.next) {
        ...
        if (includeHierarchySummary) {
            ...
            md.push(link(i18n.theme_hierarchy_view_summary(), `${this.relativeURL('hierarchy')}${this.options.getValue('fileExtension')}`));
        }
    ...
    if (model.indexSignatures?.length) {
        ...
        model.indexSignatures.forEach((indexSignature) => {
            md.push(`${!this.options.getValue('useCodeBlocks') ? '> ' : ''}${this.partials.indexSignature(indexSignature, {
                headingLevel: options.headingLevel + 1,
            })}`);
        });
    }
