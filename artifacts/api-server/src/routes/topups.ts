...
router.get("/topups", authenticate, async (req, res) => {
  ...
  const [{ total }] = await db
    .select({ total: db.$count(topupsTable) })
    .from(topupsTable)
    .where(eq(topupsTable.userId, userId)) as any;
...
// POST /topups — submit a new top-up request
...
// GET /topups/all — admin/owner: list all top-ups
...
// PATCH /topups/:id/confirm — admin/owner
router.patch("/topups/:id/confirm", authenticate, requireRole("admin", "owner"), async (req, res) => {
  ...
  logger.info({ topupId: id, userId: topup.userId, amount: topup.amount }, "Topup confirmed");
  res.json(updated);
});

// PATCH /topups/:id/deny — admin/owner
router.patch("/topups/:id/deny", authenticate, requireRole("admin", "owner"), async (req, res) => {
  ...
  logger.info({ topupId: id, userId: topup.userId, amount: topup.amount }, "Topup denied");
  res.json(updated);
});
