...
import { Search, ChevronLeft, ChevronRight, Users, RefreshCw, Wallet, Crown, Shield, Ban, CheckCircle2 } from "lucide-react";
...
export default function AdminUsers() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => adminFetch(`/users?search=${search}&role=${roleFilter !== "all" ? roleFilter : ""}&page=${page}&limit=15`),
        ...
        <div className="p-6 space-y-5 max-w-[1600px] mx-auto">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-48">
              ...
              <Input placeholder="Cari username atau email..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9" />
            </div>
                  ...
                  {isLoading
                    ...
                    : users.map((u: any) => (
                      <tr key={u.id} className="hover:bg-muted/20">
                        ...
                        <td className="p-3 text-muted-foreground hidden md:table-cell text-xs">{u.email}</td>
                        <td className="p-3">
                          <Badge className={`border text-xs ${ROLE_COLORS[u.role] ?? ROLE_COLORS.user}`}>
                            {u.role}
                          </Badge>
                        </td>
                        ...
                        <td className="p-3 hidden xl:table-cell">
                          {u.activeSubscription ? (
                            <Badge variant="default" className="text-xs">{u.activeSubscription.name}</Badge>
                          ) : (
        ...
        <Dialog open={!!roleUser} onOpenChange={() => setRoleUser(null)}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Role saat ini: <strong>{roleUser?.role}</strong></DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              ...
              <Button onClick={() => roleMut.mutate({ id: roleUser.id, role: newRole })} disabled={roleMut.isPending}>
        ...
        <Dialog open={!!walletUser} onOpenChange={() => setWalletUser(null)}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Saldo saat ini: <strong>{fmtRp(walletUser?.walletBalance)}</strong></DialogDescription>
            </DialogHeader>
            <div className="space-y-3 py-2">
              ...
              <div className="space-y-1.5">
                <Label>Alasan</Label>
                <Input placeholder="Contoh: Bonus, koreksi manual" value={walletReason} onChange={(e) => setWalletReason(e.target.value)} />
              </div>
            </div>
            <DialogFooter>
              ...
              <Button
                ...
                disabled={walletMut.isPending || !walletAmount}
              >
