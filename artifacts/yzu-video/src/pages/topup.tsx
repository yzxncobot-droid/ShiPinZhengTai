...
export default function TopupPage() {
  ...
  // Handle TemanQRIS callback redirect — the payment popup redirects to
  // /topup?order_id=...&amount=...&status=... after the user clicks
  // "Sudah Bayar". Show a toast and refresh the balance.
  useEffect(() => {
    ...
    if (orderId && status) {
      ...
      if (status === "confirmed" || status === "paid") {
        toast({
          ...
          description: "Pembayaran berhasil dikonfirmasi. Saldo kamu sudah bertambah otomatis.",
        });
        queryClient.invalidateQueries({ queryKey: getGetMeQueryKey() });
      } else if (status === "awaiting_confirmation") {
        toast({
          title: "Pembayaran Sedang Diverifikasi",
          description: "Terima kasih! Pembayaran kamu sedang diverifikasi oleh sistem.",
        });
      }
      // Clean the URL
  ...
  // Refresh balance when the window regains focus (user returns from the
  // payment popup). The webhook credits the wallet server-side; this just
  // updates the UI.
  useEffect(() => {
    const handleFocus = () => {
      queryClient.invalidateQueries({ queryKey: getGetMeQueryKey() });
  ...
  return (
    <ProtectedRoute>
      <AppLayout>
        ...
        {/* TemanQRIS Widget — moved above payment options */}
          ...
          >
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm font-medium text-slate-500">Tekan tombol di bawah untuk membuka pembayaran QRIS. Masukkan nominal di halaman pembayaran.</p>
              </div>
              <div className="rounded-2xl bg-violet-50 p-3 text-violet-600"><QrCode className="h-5 w-5" /></div>
            </div>
            <TemanQrisWidget
              merchantId="MQECF85EABA6"
              userId={user?.id}
